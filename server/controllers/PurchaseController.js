...
import fs from "fs";
const { Purchase, Course, User } = models;
...
export const downloadReceipt = async (req, res) => {
  try {
    ...
    // Use camelCase fields as your controllers expect
    const { id, title, price, createdAt } = transaction;
    const numericPrice = Number(price || 0);
    const formattedPrice = numericPrice.toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 });
    const formattedDate = new Date(createdAt).toLocaleDateString("id-ID", { day: "2-digit", month: "2-digit", year: "numeric" });

    const recipientName = transaction.User?.name || "Nama Pelanggan";
    const recipientEmail = transaction.User?.email || "Email Pelanggan";
    ...
    doc.fillColor("#ffffff").fontSize(10).text("TOTAL PAID", 390, 55, { width: 150, align: "right" });
    doc.fontSize(16).text(formattedPrice, 390, 70, { width: 150, align: "right" });
    ...
    const col3 = 370;
    const col4 = 465;
    const colWidths = { harga: 65, kuantitas: 40, total: 65 };
    ...
    doc.fillColor("#000").fontSize(10).text("THANK YOU", 450, doc.y, { align: "right" });
...
export const getUserPurchases = async (req, res) => {
  try {
    ...
    const purchases = await Purchase.findAll({
      where: { userId },
      include: [{ model: Course, attributes: ["id", "title", "price"], required: false }],
    ...
    const formatted = purchases.map((p) => ({
      id: p.id,
      courseId: p.courseId,
      title: (p.Course && p.Course.title) || p.title || "(Unknown Course)",
      price: (p.Course && p.Course.price) || p.price || 0,
...
/**
 * getAllPurchases: admin listing of all purchases
 */
...
export const downloadPurchaseReport = async (req, res) => {
  try {
    ...
    const logoPath = path.resolve("../client/public/logo.png");
    if (fs.existsSync(logoPath)) {
      try { doc.image(logoPath, 50, 45, { width: 70 }); } catch (e) { console.warn("Logo error:", e); }
    }

    doc.fontSize(22).fillColor("#1A1A1A").text("Purchase Report", 50, 55, { align: "center" });
    doc.fontSize(10).fillColor("#777").text(`Generated on: ${new Date().toLocaleString()}`, 50, 85, { align: "center" });
    ...
    doc.rect(50, tableTop, 500, headerHeight).fill("#2C3E50").fillColor("#FFFFFF").fontSize(10)
    ...
    purchases.forEach((purchase, index) => {
      ...
      doc.rect(50, y, 500, 25).fill(fillColor).fillColor("#000000").fontSize(9)
        ...
        .text(`$${(purchase.price || 0).toFixed(2)}`, 470, y + 8, { width: 50, align: "right" });
    ...
    doc.fontSize(12).fillColor("#2C3E50").text("Summary", 50, y + 25).moveDown(0.5)
    ...
    doc.moveDown(2);
    doc.fontSize(10).fillColor("#999").text("This report is restricted for internal use only.", 50, doc.y, { align: "center" });
...
export const getCoachTransactions = async (req, res) => {
  try {
    ...
    const formatted = purchases.map((p) => ({
      ...
      courseTitle: p.Course?.title || p.title || "Unknown",
      price: p.Course?.price || p.price || 0,
      date: new Date(p.createdAt).toLocaleString()
    }));
