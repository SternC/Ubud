...
const handleSave = (e) => {
  e.preventDefault();
  axios
    .put("http://localhost:5000/profile", profile, { withCredentials: true }) // trailing slash removed
    .then((res) => {
      if (res.status === 200) {
        setIsEditing(false);
      }
              ...
              <div className="absolute inset-0 bg-[#6D94C5] rounded-3xl shadow-2xl p-6 flex flex-col justify-center [transform:rotateY(180deg)] [backface-visibility:hidden]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    {!isEditing ? (
                      <>
                        <h2 className="text-xl font-bold text-white p-1">
                          {profile.name || "Name"}
                        </h2>
                      ...
                      >
                        ...
                        <input
                          ...
                          value={profile.city}
                          onChange={handleChange}
                          className="p-2 text-white rounded-lg border border-gray-300 bg-transparent"
                          placeholder="City"
                        />
                        <button
                          type="submit"
